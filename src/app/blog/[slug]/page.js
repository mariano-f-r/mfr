    ...
    <div className="flex flex-row justify-center">
      <div className="flex flex-col h-screen w-1/2 max-sm:w-screen max-md:mx-4">
        ...
        <div className="mt-4">
          ...
          <p className="mt-2 text-xl text-white text-opacity-50">{parsedData.data.desc}</p>
          <div className="flex mt-2">
            <h2 className="text-base">By {parsedData.data.author}</h2>
            <h2 className="text-base self-end ml-auto">Dated {parsedData.data.date}</h2>
          </div>
          <hr />
          <div className=" mt-2 relative w-full aspect-[16/9] overflow-hidden">
            <Image className="absolute inset-0 w-full h-full object-cover" alt="Blog image" width={16*60} height={9*60} src={parsedData.data.image} />
          </div>
